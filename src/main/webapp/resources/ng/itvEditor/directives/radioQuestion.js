editorApp.directive('radioQuestion', function(editOptionService, editQuestionService){
	return{
		restrict: 'E',
		scope: {
			question: '=',
		...
		templateUrl: '../resources/ng/itvEditor/directives/radioQuestion.html',
		controller: function($scope, $element, $modal){
			$scope.$watch("directiveAnswer", function() {
				$scope.showAnswer = $scope.directiveAnswer;
			});
			...
			// set edit option icon to visible when mouse enters
			$scope.showEdit = function(optNum){
				$scope.editOptionVisible[optNum] = true;
			};
			// set edit option icon to invisible when mouse leaves
